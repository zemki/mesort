      ...
      <div class="z-50 w-1/3 mx-auto bg-white modal-container">
        ...
        <div class="w-auto px-6 py-4 text-left modal-content">
          ...
          <div class="w-2/3 mx-auto">
            <img :src="newtoken.base64" alt="person" class="m-auto w-25" />
      ...
      <div class="flex w-auto h-auto">
        ...
        >
          ...
          >
            ...
            <img :src="t.image_path" alt="token image" class="token-image" />
            <img
              v-for="(classifiersimages, cindex) in t.valutation.classifiers"
              :key="cindex"
            ...
            >
              ...
              <div
                v-if="t.drag"
                class="inline-block text-sm text-white bg-blue-500 rounded-none"
                v-html="'Center Distance ' + t.valutation.distance"
              ></div>
              <div
                v-if="t.drag"
                class="inline-block text-sm text-white bg-blue-500 rounded-none"
              ...
              <div
                ...
                class="inline-block text-sm text-white bg-blue-500 rounded-none"
                v-html="'% position X ' + t.percentagePosition.x"
              ></div>
              <div
                v-if="t.drag && t.percentagePosition"
                class="inline-block text-sm text-white bg-blue-500 rounded-none"
...
export default {
      ...
      if (newVal) {
        axios
          ...
          .then((response) => {
            ...
            this.showToast("Person was successfully added.", "success", 1000);
  ...
  methods: {
    ...
    savetoken(close) {
      ...
      self.newtoken.file =
        self.baseUrl + self.productionUrl + self.newtoken.file;
      if (self.newtoken.name !== "") {
        axios
          ...
          .then((response) => {
            ...
            self.newtoken.name = "";
            self.newtoken.file = "/images/presets_tokens/person.png";
            if (close === "close") {
    ...
    assignclassifier: function (tokenid, index) {
      if (this.classifiermode) {
        let classifierindex = this.tokens[
          index
        ].valutation.classifiers.findIndex(
          (classifier) =>
            classifier === this.$store.state.newinterview.selectedclassifier
        );
    ...
    deletetoken: function (token) {
      axios
        .post("../api/v1/deletetoken", {
          study: this.tokens[0].pivot.study_id,
          token: token,
        })
        .then((response) => {
          let IndexTokenToDelete = this.tokens.findIndex(function (
            currentObject
          ) {
    ...
    calculateTokenValues: function (event, self) {
      ...
      let elementNewPosition = self.getElementGlobalCoordinates(event.target),
        currentX = elementNewPosition.left,
        currentY = elementNewPosition.top + sortingInstructionPosition.height;

      let position = self.getTokenCircleAndCenterDistance(elementNewPosition);
          ...
          if (!self.classifiermode) {
            ...
            for (let i = 0; i < self.sectionNumber; i++) {
              ...
              if (
                ...
                self.tokens[tokenArrayIndex].valutation.section =
                  "outside the circle";
              }
            }
    ...
    calculateCircleCenter: function () {
      this.center_x =
        this.bounds.left +
        document.querySelector(".round-sorting" + this.circles).offsetWidth / 2;
      this.center_y =
        document.querySelector(".round-sorting" + this.circles).offsetHeight /
          2 +
        this.bounds.top;
    },
    getbounds: function () {
      ...
      setTimeout(function () {
        if (self.currentsorting == 1) {
          self.parentPos = document
            .querySelector(".sorting-container" + self.currentsorting)
            .getBoundingClientRect();
